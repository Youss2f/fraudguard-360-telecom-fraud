...
export function LocalCallActivityCard({ data, filters, onImeiHighlight, highlightedImei }: LocalCallActivityCardProps) {
  ...
  const weekdayWeekendData = [
    {
      name: "Weekday",
      calls: data.weekdayWeekendSplit.weekday.calls,
      duration: data.weekdayWeekendSplit.weekday.duration,
    },
    {
      name: "Weekend",
      calls: data.weekdayWeekendSplit.weekend.calls,
      duration: data.weekdayWeekendSplit.weekend.duration,
    },
  ]
          ...
          <TabsContent value="overview" className="space-y-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="text-center p-3 bg-blue-50 rounded-lg">
                ...
                <div className="text-2xl font-bold text-blue-600">{data.stats.totalCalls.outgoing}</div>
                <div className="text-sm text-gray-600">Outgoing</div>
              </div>

              <div className="text-center p-3 bg-green-50 rounded-lg">
                <PhoneIncoming className="h-6 w-6 mx-auto mb-2 text-green-600" />
                <div className="text-2xl font-bold text-green-600">{data.stats.totalCalls.incoming}</div>
                <div className="text-sm text-gray-600">Incoming</div>
              </div>

              <div className="text-center p-3 bg-purple-50 rounded-lg">
                <Clock className="h-6 w-6 mx-auto mb-2 text-purple-600" />
                <div className="text-2xl font-bold text-purple-600">{formatDuration(data.stats.avgDuration)}</div>
                <div className="text-sm text-gray-600">Avg Duration</div>
              </div>

              <div className="text-center p-3 bg-orange-50 rounded-lg">
                <Users className="h-6 w-6 mx-auto mb-2 text-orange-600" />
                <div className="text-2xl font-bold text-orange-600">{data.stats.uniqueContacts}</div>
