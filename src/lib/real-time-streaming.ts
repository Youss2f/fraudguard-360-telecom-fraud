...
class RealTimeDataStream extends EventEmitter {
  ...
  start(): void {
    if (this.isActive) {
      // Real-time stream is already active
      return
    }
  ...
  stop(): void {
    if (!this.isActive) {
      // Real-time stream is not active
      return
    }
...
export { globalStream as realTimeStream }
export type { RealTimeDataStream, StreamEvent }
