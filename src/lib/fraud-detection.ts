  ...
  try {
    ...
    for (let i = 0; i < recentCalls.length - 1; i++) {
      ...
      if (call1.latitude && call1.longitude && call2.latitude && call2.longitude) {
        const distance = calculateDistance(call1.latitude, call1.longitude, call2.latitude, call2.longitude)
...
export async function detectFraud(subscriberId: string): Promise<FraudDetectionResult> {
  ...
  try {
    ...
    const confidence =
      alerts.length > 0 ? alerts.reduce((sum, alert) => sum + alert.confidence, 0) / alerts.length : 0.95
...
export { FRAUD_CONFIG }
export type { FraudDetectionResult, FraudAlert, BehaviorPattern }
